...
#define WIFI_DRIVER_MODULE_PATH         "/data/modules/wlan.ko"
#endif
...
static int devHandle = FILE_HANDLE_INVALID;

//~ return dev file handle on success, -1 on error
static int power_on()
{
	int ret = devHandle;

    if (FILE_HANDLE_INVALID != devHandle) {
        ALOGE("[MT6620]already opened, handle = %d\n", devHandle);
        goto end;
    }

	ret = open(wmtWifi, O_RDONLY);
	if(FILE_HANDLE_INVALID == ret) {
	    ALOGE("[MT6620]open failed %s\n", strerror(errno));
        goto end;
    }
	
	devHandle = ret;
    ALOGE("[MT6620]handle = %d\n", devHandle);
...
static int power_off()
{
	...
	if(FILE_HANDLE_INVALID != devHandle){
		ret = close(devHandle);
		devHandle = FILE_HANDLE_INVALID;
	}
	
	if(0 != ret)
	    ALOGE("[MT6620]close failed %s\n", strerror(errno));
...
static int check_driver_loaded() {
    ...
    if (!property_get(DRIVER_PROP_NAME, driver_status, NULL)
        ...
        ALOGW("check_driver_loaded(): %s = %s\n", DRIVER_PROP_NAME, driver_status);
     ...
     * crash.
     */
#if 1 // Begin of MT6620 code
    if (FILE_HANDLE_INVALID != devHandle) {
        ALOGW("check_driver_loaded(): found %s!\n", DRIVER_MODULE_TAG);
        return 1;
    } else {
...
#if 1	//0 : only wlan, 1 : all 
    // In the init.rc, we will load the wlan.ko permanently.
    if (power_on() < 0)  //power on the wlan after the loading of the wifi.
        return -1;
#else
...
{
#if 1 // Begin of MT6620 code	0 : only wlan, 1 : all 
    return power_off();
#else   
