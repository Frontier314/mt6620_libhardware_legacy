...
int set_iface(const char *iface, int on)
{
    ...
    if(ifc_init() != 0) {
        ALOGE("[%s] interface set %d failed", iface, on);
        return -1;
    }
    if(on) {
        while(ifc_up(iface) == -1) {
            ALOGD("[%s] interface is not ready, wait %dus", iface, SET_IFACE_DELAY);
            sched_yield();
        ...
        ALOGD("[%s] interface is up", iface);
        init_iface(iface);
    }
    else {
        ifc_down(iface);
        ALOGD("[%s] interface is down", iface);
    }
    ifc_close();
    return 0;
}
...
static int ensure_config_file_exists(const char *configFile, const char *configTemplate)
{
    ...
    if (access(configFile, R_OK|W_OK) == 0) {
        ...
        ALOGE("Cannot access \"%s\": %s", configFile, strerror(errno));
        return -1;
    }

    srcfd = open(configTemplate, O_RDONLY);
    if (srcfd < 0) {
        ALOGE("Cannot open \"%s\": %s", configTemplate, strerror(errno));
        return -1;
    }
    ...
    if (destfd < 0) {
        close(srcfd);
        ALOGE("Cannot create \"%s\": %s", configFile, strerror(errno));
        return -1;
    }

    while ((nread = read(srcfd, buf, sizeof(buf))) != 0) {
        if (nread < 0) {
            ALOGE("Error reading \"%s\": %s", configTemplate, strerror(errno));
            close(srcfd);
            close(destfd);
            unlink(configFile);
            return -1;
...
{
    ALOGD("[%s] Close CTRL connection", prTarInfo->acIfName);
...
void close_supplicant_connection(const struct _SUPPLICANT_PARA_T * prTar, P_SUPPLICANT_INFO_T prTarInfo)
{
    ...
    ALOGD("[%s] Close CTRL & MONITOR connection", prTarInfo->acIfName);
    if (prTarInfo->prCtrlConn != NULL) {
        prTarInfo->rSuppCtrl.ctrl_close(prTarInfo->prCtrlConn);
        prTarInfo->prCtrlConn = NULL;
    }
...
int start_supplicant(const struct _SUPPLICANT_PARA_T * prTar, const char *pcIface)
{
    ...
    if (ensure_config_file_exists(prTar->acSuppConfigFile, prTar->acSuppConfigTemplate) < 0) {
        ALOGE("[%s] %s will not be enabled", pcIface, prTar->acSuppName);
        return -1;
    }
    ...
    ALOGD("[%s] clear out stale sockets with prefix \"%s\" in %s", pcIface, prTar->acSuppCtrlPrefix, CONFIG_CTRL_IFACE_CLIENT_DIR);
    ctrl_cleanup(prTar->acSuppCtrlPrefix);

    ALOGI("[%s] start %s", pcIface, prTar->acSuppName);
#ifdef HAVE_LIBC_SYSTEM_PROPERTIES
    /*
    ...
    ALOGD("[%s] supplicant start command: \"%s\"", pcIface, daemon_cmd);
...
int stop_supplicant(const struct _SUPPLICANT_PARA_T *prTar, P_SUPPLICANT_INFO_T prTarInfo)
{
    ...
    ALOGI("[%s] stop %s", prTarInfo->acIfName, prTar->acSuppName);
...
int connect_to_supplicant(const struct _SUPPLICANT_PARA_T *prTar, P_SUPPLICANT_INFO_T prTarInfo)
{
    ...
    if (!property_get(prTar->acSuppPropName, supp_status, NULL)
            || strcmp(supp_status, "running") != 0) {
        ALOGE("[%s] %s not running, cannot connect", prTarInfo->acIfName, prTar->acSuppName);
        return -1;
    }

    ALOGI("[%s] Connect to %s", prTarInfo->acIfName, prTar->acSuppName);

    snprintf(ifname, sizeof(ifname), "%s/%s", prTar->acIfDir, prTarInfo->acIfName);
	ALOGD("[%s] ctrl_open %s", prTarInfo->acIfName, ifname);
    {
        int count = CONNECT_TO_SUPP_POLLING_LOOP;
        while (count-- > 0) {
            prTarInfo->prCtrlConn = prTarInfo->rSuppCtrl.ctrl_open(ifname);
            if (prTarInfo->prCtrlConn) {
                ALOGD("[%s] Open control connection to %s successfully\n", prTarInfo->acIfName, ifname);
                break;
            }
            ALOGD("[%s] Open control connection fail (%s). Sleep %dus\n", prTarInfo->acIfName, ifname, CONNECT_TO_SUPP_DELAY);
            sched_yield();
    ...
    if (prTarInfo->prCtrlConn == NULL) {
        ALOGE("[%s] Unable to open connection to supplicant on \"%s\": %s"
            , prTarInfo->acIfName
            , ifname
            , strerror(errno));
        return -1;
    ...
    ALOGD("[%s] Connect_to_supplicant %s successfully.\n", prTarInfo->acIfName, ifname);
...
int ctrl_recv(P_SUPPLICANT_INFO_T prTarInfo, char *reply, size_t *reply_len)
{
    ...
    res = poll(rfds, 2, -1);
    if (res < 0) {
        ALOGE("[%s] Error poll = %d", prTarInfo->acIfName, res);
        return res;
    }
    if (rfds[0].revents & POLLIN) {
        return prTarInfo->rSuppCtrl.ctrl_recv(prTarInfo->prMonitorConn, reply, reply_len);
    } else {
        ALOGD("[%s] Received on exit socket, terminate", prTarInfo->acIfName);
        return -1;
    }
    return 0;
}
...
int do_dhcp_client_request(const char *iface, int *ipaddr, int *gateway, int *mask,
    ...
    ALOGD("[%s] do_dhcp_client_request\n", iface);
